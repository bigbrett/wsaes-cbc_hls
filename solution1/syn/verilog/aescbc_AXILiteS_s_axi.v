...
assign int_data_out_we0      = data_out_we0;
assign int_data_out_be0      = 1 << data_out_address0[1:0];
...
always @(posedge ACLK) begin
    if (ACLK_EN) begin
        if (data_in_ce0)
            int_data_in_shift <= data_in_address0[1:0];
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_data_out_write <= 1'b0;
    else if (ACLK_EN) begin
        if (aw_hs && AWADDR[ADDR_BITS-1:0] >= ADDR_DATA_OUT_BASE && AWADDR[ADDR_BITS-1:0] <= ADDR_DATA_OUT_HIGH)
            int_data_out_write <= 1'b1;
        else if (WVALID)
            int_data_out_write <= 1'b0;
    end
end
...
module aescbc_AXILiteS_s_axi_ram
#(parameter
    BYTES  = 4,
    DEPTH  = 256,
    AWIDTH = log2(DEPTH)
