...
entity aescbc_AXILiteS_s_axi is
generic (
...
architecture behave of aescbc_AXILiteS_s_axi is
    ...
    end component aescbc_AXILiteS_s_axi_ram;
...
port map (
     ...
     q1       => int_data_in_q1);
-- int_data_out
int_data_out : aescbc_AXILiteS_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 4,
     AWIDTH   => log2(4))
port map (
     ...
     we0      => int_data_out_we0,
     be0      => int_data_out_be0,
     d0       => int_data_out_d0,
     q0       => int_data_out_q0,
     clk1     => ACLK,
     address1 => int_data_out_address1,
     ce1      => int_data_out_ce1,
     we1      => int_data_out_we1,
...
-- ----------------------- Memory logic ------------------
    ...
    int_data_out_we0     <= data_out_we0;
    int_data_out_be0     <= SHIFT_LEFT(TO_UNSIGNED(1, 4), TO_INTEGER(UNSIGNED(data_out_address0(1 downto 0))));
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (data_in_ce0 = '1') then
                    int_data_in_shift <= UNSIGNED(data_in_address0(1 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_data_out_write <= '0';
            elsif (ACLK_EN = '1') then
                if (aw_hs = '1' and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) >= ADDR_DATA_OUT_BASE and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) <= ADDR_DATA_OUT_HIGH) then
                    int_data_out_write <= '1';
                elsif (WVALID = '1') then
                    int_data_out_write <= '0';
                end if;
            end if;
...
entity aescbc_AXILiteS_s_axi_ram is
    generic (
        BYTES   : INTEGER :=4;
        DEPTH   : INTEGER :=256;
        AWIDTH  : INTEGER :=8);
...
end entity aescbc_AXILiteS_s_axi_ram;

architecture behave of aescbc_AXILiteS_s_axi_ram is
