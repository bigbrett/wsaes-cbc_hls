...
int main( int argc, char *argv[]) {
	...
	// Test our engine
	aescbc(key, SET_KEY, 0, 0);
	aescbc(iv, SET_IV, 0, 0);
	aescbc(vRAM, RESET, 0, 0);
	aescbc(vRAM, ENCRYPT, 0, 32);
	aescbc(vRAM, ENCRYPT, 16, 48);
...
//	if (strncmp((const char*)&(vRAM[32]), openSSL_result, 32))
	if (memcmp(&(vRAM[32]), openSSL_result, 32))
	{
	...
	// Decrypt
	aescbc(vRAM, RESET, 0, 0);
	aescbc(vRAM, DECRYPT, 32, 0);
	aescbc(vRAM, DECRYPT, 48, 16);
